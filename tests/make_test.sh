...
case "$1" in
    ...
    "ARM64") ARCH_FLAG=""; shift ;;
    "RISC-V") ARCH_FLAG=""; shift ;;
esac

WARN_FLAGS="-Wall -Wno-misleading-indentation"
FNO_FLAGS="-fno-exceptions -fno-rtti -fwhole-program"
OPT_FLAGS="-O2 -std=$STDCXX $FNO_FLAGS"
...
case "$mode" in
	...
	"FAST") echo "Make with -O3 -march=native..."; $gccbin $WARN_FLAGS -O3 -std=$STDCXX $FNO_FLAGS "$cppfile" -o "$exefile" -march=native -mtune=native -flto -funroll-loops -fipa-cp-clone -ffast-math -fomit-frame-pointer $LNK_FLAGS ;;
	"SAN") echo "Make using sanitizers..."; $gccbin $WARN_FLAGS -g -Og -std=$STDCXX $FNO_FLAGS "$cppfile" -o "$exefile" -lpthread -fsanitize=address,undefined ;;
	"DBG") echo "Make debug ver -g -Og..."; $gccbin $WARN_FLAGS -g -Og -std=$STDCXX $FNO_FLAGS "$cppfile" -o "$exefile" -lpthread ;;
	...
	"EXP_SAN") echo "Make using sanitizers (for Experimental)..."; $gccbin -DUSE_GUI -DUSE_EXPERIMENTAL $WARN_FLAGS -g -Og -std=$STDCXX $FNO_FLAGS "$cppfile" -o "$exefile" -lGL -lXtst -lX11 -lasound -lpthread -fsanitize=address,undefined ;;
