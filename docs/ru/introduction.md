...
make.sh С++98 x32 proj2
make.sh С++17 x64 GUI proj5
```
...
- **Режимы сборки**:
	...
	- `MUSL`: (Только Linux) Использовать линковку с `musl` для проекта (может требоваться пакет `musl-tools` для вызова `musl-gcc`)
...
Поддерживается указание только одного из режимов. Режимы сборки последовательно подключают реалазации в следующем порядке: 
`(Минимальная реализация) DEF -> GUI -> GL -> EXP (Максимальная реализация)`.
...
### Предопределённые макросы
 - `NCPP_VER` - строка с версией библиотеки
 
Макросы, вырезающие часть реализации:
 - `NCPP_BASE_ONLY` - включить только модуль `Base`.
 - `NCPP_SYS_ONLY` - включить только модули `Base` + `System`.
 - `NOUSE_NET` - исключить модуль сети `Net`.
 - `NOUSE_OBJECT` - исключить реализацию `ncpp::Object` вместе с реализациями `JSON/CBOR/BSON`.
 - `NOUSE_MEDIA` - исключить реализации мультмедиа (Изображения, Аудио, etc.)
 - `NOUSE_CRYPTO` - исключить реализации криптографии.
 - `NOUSE_GL` - не включать реализацию расширеной графики (OpenGL/Vulkan) при включении модуля `GUI`.
 
Иные макросы:
 - `USE_GUI` - включить реализацию графического интерфейса (модуль `GUI`).
 - `USE_EXPERIMENTAL` - включить экспериментальные реализации.
